...
const TEMPLATES = [
  {
    ...
    accentColor: '#FCA5A5',
    description: 'Something breaks at 3am — this Room notices, figures out what failed, and wakes the right person. Before your customers do.',
    what: [
      'Polls every health endpoint every 60 seconds',
      'Classifies what broke and why',
      'Alerts the right person with context already written',
    ],
    trigger: 'Runs on schedule — or fires from any alert webhook',
    workflow: { name: 'Watchdog Flow', agentGoal: 'Check the health status of all registered service endpoints, detect any degradation or failure, classify the severity and likely root cause, then immediately notify the on-call contact via the configured connector with a structured incident summary.' },
  },
  {
    id: 'help-desk',
    ...
    accentColor: '#93C5FD',
    description: 'Every support message gets read, understood, and answered in seconds. Hard ones get escalated with full context already attached.',
    what: [
      'Reads and classifies every incoming message',
      'Finds the right answer from your knowledge base',
      'Replies instantly — or escalates with full context',
    ],
    trigger: 'Webhook — from any inbox, chat, or support tool',
    workflow: { name: 'Help Desk Flow', agentGoal: 'Receive the incoming support message, classify its intent and urgency, search the knowledge base for matching resolutions, then either draft and send a complete response or escalate to the right human with a detailed context summary already written.' },
  },
  {
    id: 'model-relay',
    Icon: ReportsIcon,
    ...
    accentColor: '#6EE7B7',
    description: 'Chain different AI models in one pipeline. Each handles the step it\'s best at — cheaper, faster, and smarter together.',
    what: [
      'Model 1 extracts and structures raw input',
      'Model 2 reasons, fact-checks, and enriches',
      'Final agent delivers polished output downstream',
    ],
    trigger: 'Webhook or schedule — any signal fires the chain',
    workflow: { name: 'Model Relay Flow', agentGoal: 'Receive the input, route it through the first model to extract and structure the key data, pass that output to the second model to reason and fact-check, then synthesise a final polished output and deliver it via the configured output connector.' },
  },
  {
    id: 'data-bridge',
    Icon: APIIcon,
    ...
    accentColor: '#A5B4FC',
    description: 'Pull from any API, clean the data, merge it, and push it where it needs to go. No scripts, no glue code, no babysitting.',
    what: [
      'Fetches from every configured source API',
      'Cleans, merges, and resolves conflicts automatically',
      'Delivers a unified record to any destination',
    ],
    trigger: 'Webhook — triggered by any upstream change',
    workflow: { name: 'Data Bridge Flow', agentGoal: 'Fetch data from all configured source API connectors, validate and clean each record, apply field mapping and transformation rules, resolve any conflicts, and deliver a unified structured output to the configured destination connector.' },
  },
  {
    id: 'audit-sweep',
    Icon: ComplianceIcon,
    ...
    accentColor: '#FCD34D',
    description: 'Every output your system produces gets checked against your rules. Violations get flagged. Nothing slips through.',
    what: [
      'Ingests any output or log via webhook',
      'Checks it against your stored policy rules',
      'Scores compliance and flags every violation',
    ],
    trigger: 'Webhook — from any system producing auditable output',
    workflow: { name: 'Audit Sweep Flow', agentGoal: 'Receive the content or log payload, evaluate it against the stored policy ruleset, score overall compliance from 0 to 100, quote any specific violations with rule references, and generate a structured audit report with recommended remediation for each issue.' },
  },
  {
    id: 'deep-dive',
    ...
    what: [
      'Breaks the topic into parallel research threads',
      'Each agent owns a sub-topic, writes to shared memory',
      'One final agent synthesises everything into a report',
    ],
    trigger: 'API call or webhook — pass any question as the payload',
    workflow: { name: 'Deep Dive Flow', agentGoal: 'Decompose the research question into sub-topics, spawn parallel research threads, gather evidence from configured sources for each sub-topic, store findings in shared Room memory, then synthesise all findings into a single structured research report with citations and key conclusions.' },
  },
]
